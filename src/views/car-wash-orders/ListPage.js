...
import { getAllCarWashOrders, getCarWashOrderById, postCarWashOrders, updateCarWashOrders, deleteCarWashOrderById } from "../../services/car-wash/car-wash-orders-service.js";
...
function ListPage({ loggedInUserData, selectedParentItem, ...props }) {
    ...
    const { data, isLoading, isError, error, status } = useQuery(["car-wash-order"], getAllCarWashOrders, {
        onSuccess: (data) => {
            console.log("list of all car-wash-order : ", data);
        },
        onError: (error) => {
    ...
    const DeleteSelectedItemMutation = useMutation((variables) => deleteCarWashOrderById(variables), {
        onSuccess: (data) => {
            queryClient.invalidateQueries(["car-wash-order"]);
        },
        onError: (error) => {
            error?.response?.data?.message ? toast.error(error?.response?.data?.message) : !error?.response ? toast.warning("Check Your Internet Connection Please") : toast.error("An Error Occured Please Contact Admin");
        },
    ...
    const columns = [
        ...
        {
            title: "Currency",
            field: "car_wash_fee.currency",
            render: (rowData) => rowData?.car_wash_fee?.currency || "No Currency",
        },
        {
            title: "Billing Cycle",
            field: "car_wash_fee.billing_cycle",
            render: (rowData) => rowData?.car_wash_fee?.billing_cycle || "No Billing Cycle",
        },
        // {
        //     title: "Status",
        //     field: "status",
        ...
        {
            title: "Fee Amount",
            field: "car_wash_fee.fee_amount",
            render: (rowData) => {
                return rowData?.car_wash_fee?.fee_amount ? parseFloat(rowData?.car_wash_fee?.fee_amount).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : "No Fee";
            },
        },
    ...
    return (
        <div style={{ width: "100%" }}>
            ...
            <Panel header="Car Wash" style={{ marginBottom: "20px" }}>
                <div style={{ height: "3rem", margin: "1rem", display: "flex", justifyContent: "flex-end", gap: "1rem" }}>
                    {activeUser?.permissions.includes("create") && <Button label="Add Car Wash Order" className="p-button-primary" onClick={() => setShowAddForm(true)} />}
                ...
                <MuiTable
                    ...
                    //
                    exportButton={true}
                    pdfExportTitle="Car Wash Services"
                    csvExportTitle="Car Wash Services"
