...
function RowForm({ handleSubmit, selectedParentItem, initialData = { name: "", details: "", photoUrl: "" }, ...props }) {
    ...
    //================================== Cars (Cars) =============================
    const [selectedCar, setSelectedCar] = useState(initialData?.car);
    ...
    //================================== Car Wash Fees (CarWashFee) =============================
    const [selectedCarWashFee, setSelectedCarWashFee] = useState(initialData?.car_wash_fee);
    const [filteredCarWashFee, setFilteredCarWashFee] = useState();

    const getListOfCarWashFees = useQuery(["car-wash-fees"], () => getAllCarWashFees(), {
        onSuccess: (data) => {},
        onError: (error) => {
            console.log("Error fetching Car Wash Fees: ", error);
            error?.response?.data?.message ? toast.error(error?.response?.data?.message) : !error?.response ? toast.warning("Check Your Internet Connection Please") : toast.error("An Error Occured Please Contact Admin");
        },
    });
            ...
            <div className="card p-fluid">
                <Form
                    ...
                    render={({ handleSubmit, form, submitting, pristine, values }) => (
                        ...
                        >
                            <Field name="car_wash_fee_id">
                                {({ input, meta }) => (
                                    <div className="p-field m-4">
                                        <label htmlFor="car_wash_fee_id">Car Wash Fee</label>
                                        <AutoComplete
                                            value={selectedCarWashFee?.name || ""}
                                            suggestions={filteredCarWashFee}
                                            disabled={getListOfCarWashFees.isLoading}
                                            completeMethod={(e) => {
                                                const results = getListOfCarWashFees.data?.data?.filter((item) => {
                                                    return item.name.toLowerCase().includes(e.query.toLowerCase());
                                                });
                                                setFilteredCarWashFee(results);
                                            }}
                                            field="name"
                                            dropdown={true}
                                            onChange={(e) => {
                                                if (typeof e.value === "string") {
                                                    ...
                                                    setSelectedCarWashFee({ name: e.value });
                                                    input.onChange("");
                                                } else if (typeof e.value === "object" && e.value !== null) {
                                                    // Update the selected item and set the form state with the selected item's ID
                                                    setSelectedCarWashFee(e.value);
                                                    input.onChange(e.value.id);
                                                }
                                            }}
                                            id="car_wash_fee_id"
                                            selectedItemTemplate={(value) => <div>{value ? value.name : "Select a Car Wash Fee"}</div>}
                                        />
                                        {meta.touched && meta.error && <small className="p-error">{meta.error}</small>}
                                        {getListOfCarWashFees.isLoading && <ProgressSpinner style={{ width: "10px", height: "10px" }} strokeWidth="4" />}
                                    </div>
                                )}
                            </Field>
