...
import { getAllDashboardSliderPhotos, getDashboardSliderPhotoById, postDashboardSliderPhotos, updateDashboardSliderPhotos, deleteDashboardSliderPhotoById } from "../../../services/dashboardslider/dashboard-slider-photos-services.js";
...
function ListPage({ loggedInUserData, ...props }) {
    ...
    const { data, isLoading, isError, error, status } = useQuery(["db-slider-photos"], getAllDashboardSliderPhotos, {
        onSuccess: (data) => {
    ...
    const DeleteSelectedItemMutation = useMutation((variables) => deleteDashboardSliderPhotoById(variables), {
        onSuccess: (data) => {
            queryClient.invalidateQueries(["db-slider-photos"]);
        },
        onError: (error) => {
            error?.response?.data?.message ? toast.error(error?.response?.data?.message) : !error?.response ? toast.warning("Check Your Internet Connection Please") : toast.error("An Error Occured Please Contact Admin");
        },
    ...
    const columns = [
...

        {
            ...
            field: "photo_url",
            render: (rowData) => {
                return rowData.photo_url ? <Image src={`${process.env.REACT_APP_API_BASE_URL}${rowData.photo_url}`} alt={rowData.caption} width="100" preview style={{ verticalAlign: "middle" }} /> : <div>No Image</div>;
            },
        },
        {
            title: "Status",
    ...
    return (
        <div style={{ width: "100%" }}>
            ...
            <Panel header="Dashbord Slider Photos" style={{ marginBottom: "20px" }}>
                <div style={{ height: "3rem", margin: "1rem", display: "flex", justifyContent: "flex-end", gap: "1rem" }}>
                    {activeUser?.permissions.includes("create") && <Button label="Add Photo" className="p-button-primary" onClick={() => setShowAddForm(true)} />}
                ...
                <MuiTable
                    tableTitle="Dashbord Slider Photos"
                    tableData={data?.data}
