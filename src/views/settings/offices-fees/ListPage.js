...
function ListPage({ loggedInUserData, ...props }) {
    ...
    const { data, isLoading, isError, error, status } = useQuery(["office-fees"], getAllOfficeFees, {
        onSuccess: (data) => {
    ...
    const DeleteSelectedItemMutation = useMutation((variables) => deleteOfficeFeeById(variables), {
        onSuccess: (data) => {
            queryClient.invalidateQueries(["office-fees"]);
        },
        onError: (error) => {
            error?.response?.data?.message ? toast.error(error?.response?.data?.message) : !error?.response ? toast.warning("Check Your Internet Connection Please") : toast.error("An Error Occured Please Contact Admin");
        },
    ...
    return (
        <div style={{ width: "100%" }}>
            ...
            <Panel header="OfFice fees" style={{ marginBottom: "20px" }}>
                <div style={{ height: "3rem", margin: "1rem", display: "flex", justifyContent: "flex-end", gap: "1rem" }}>
                    {activeUser?.permissions.includes("create") && <Button label="Add Office Fee" className="p-button-primary" onClick={() => setShowAddForm(true)} />}
