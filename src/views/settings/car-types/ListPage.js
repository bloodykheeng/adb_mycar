...
function ListPage({ loggedInUserData, ...props }) {
    ...
    const { data, isLoading, isError, error, status } = useQuery(["car-types"], getAllCarTypes, {
        onSuccess: (data) => {
            console.log("list of all car : ", data);
        },
        onError: (error) => {
    ...
    const DeleteSelectedItemMutation = useMutation((variables) => deleteCarTypeById(variables), {
        onSuccess: (data) => {
    ...
    return (
        <div style={{ width: "100%" }}>
            ...
            <Panel header="Car Types" style={{ marginBottom: "20px" }}>
                <div style={{ height: "3rem", margin: "1rem", display: "flex", justifyContent: "flex-end", gap: "1rem" }}>
                    {activeUser?.permissions.includes("create") && <Button label="Add Car Type" className="p-button-primary" onClick={() => setShowAddForm(true)} />}
