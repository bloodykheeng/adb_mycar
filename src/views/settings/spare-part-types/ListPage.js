...
function ListPage({ loggedInUserData, ...props }) {
    ...
    const { data, isLoading, isError, error, status } = useQuery(["spare-part-types"], getAllSparePartTypeTypes, {
        onSuccess: (data) => {
            console.log("list of all SparePartTypes : ", data);
        },
        onError: (error) => {
            console.log("Error fetching getAllSparePartTypes is : ", error);
    ...
    const DeleteSelectedItemMutation = useMutation((variables) => deleteSparePartTypeById(variables), {
        onSuccess: (data) => {
    ...
    return (
        <div style={{ width: "100%" }}>
            ...
            <Panel header="Spare Part Types" style={{ marginBottom: "20px" }}>
                <div style={{ height: "3rem", margin: "1rem", display: "flex", justifyContent: "flex-end", gap: "1rem" }}>
                    {activeUser?.permissions.includes("create") && <Button label="Add Spare Part Type" className="p-button-primary" onClick={() => setShowAddForm(true)} />}
