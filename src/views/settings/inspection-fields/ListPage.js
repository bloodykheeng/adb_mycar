...
function ListPage({ loggedInUserData, ...props }) {
    ...
    const { data, isLoading, isError, error, status } = useQuery(["inspection-fields"], getAllInspectionFields, {
        onSuccess: (data) => {
    ...
    const DeleteSelectedItemMutation = useMutation((variables) => deleteInspectionFieldById(variables), {
        onSuccess: (data) => {
    ...
    return (
        <div style={{ width: "100%" }}>
            ...
            <Panel header="Inspection Fields" style={{ marginBottom: "20px" }}>
