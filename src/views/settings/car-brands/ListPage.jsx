...
function ListPage({ loggedInUserData, ...props }) {
    ...
    const { data, isLoading, isError, error, status } = useQuery(["car-brands"], getAllCarBrands, {
        onSuccess: (data) => {
    ...
    const DeleteSelectedItemMutation = useMutation((variables) => deleteCarBrandById(variables), {
        onSuccess: (data) => {
            queryClient.invalidateQueries(["car-brands"]);
        },
        onError: (error) => {
            error?.response?.data?.message ? toast.error(error?.response?.data?.message) : !error?.response ? toast.warning("Check Your Internet Connection Please") : toast.error("An Error Occured Please Contact Admin");
        },
    ...
    const columns = [
...

        {
            ...
            field: "logo_url",
            render: (rowData) => {
                return rowData.logo_url ? <Image src={`${process.env.REACT_APP_API_BASE_URL}${rowData.logo_url}`} alt={rowData.name} width="100" preview style={{ verticalAlign: "middle" }} /> : <div>No Image</div>;
            },
        },
        {
            title: "Status",
    ...
    return (
        <div style={{ width: "100%" }}>
            ...
            <Panel header="Car Brand" style={{ marginBottom: "20px" }}>
                <div style={{ height: "3rem", margin: "1rem", display: "flex", justifyContent: "flex-end", gap: "1rem" }}>
                    {activeUser?.permissions.includes("create") && <Button label="Add Car Brand" className="p-button-primary" onClick={() => setShowAddForm(true)} />}
