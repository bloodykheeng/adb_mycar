...
function ListPage({ selectedCarData, loggedInUserData, ...props }) {
    ...
    console.log("selectedCarData ggggg : ", selectedCarData);

    const { data, isLoading, isError, error, status } = useQuery(
        ["car-inspection-reports", selectedCarData?.id, "pending-approval-by-the-admin"],
        () =>
    ...
    return (
        <div style={{ width: "100%" }}>
            <Panel header="Car Inspection Reports Pending Approval" style={{ marginBottom: "20px" }}>
                ...
                <ApprovalModal approvalStatus={showApprovalModalStatus} selectedCarData={selectedCarData} selectedItem={selectedItem} loggedInUserData={loggedInUserData} show={showApprovalModal} onHide={() => setShowApprovalModal(false)} onClose={handleCloseApprovalModal} />
