...
function ListPage({ selectedCarData, loggedInUserData, ...props }) {
    const navigate = useNavigate();
    const queryClient = useQueryClient();

    const { data, isLoading, isError, error, status } = useQuery(["car-inspection-reports", selectedCarData?.id], () => getAllCarInspectionReports({ car_id: selectedCarData?.id }), {
        onSuccess: (data) => {
            console.log("fetching car-inspection-reports data is : ", data);
        },
        onError: (error) => {
            console.log("Error fetching car-inspection-reports is : ", error);
        },
    });
    console.log(data);
    ...
    const DeleteSelectedItemMutation = useMutation((variables) => deleteCarInspectionReportById(variables), {
        ...
        onError: (error) => {
            ...
            console.log("error deleting car-inspection-reports error:", error);
        },
    });
    ...
    return (
        <div style={{ width: "100%" }}>
            <Panel header="Car Inspection Reports" style={{ marginBottom: "20px" }}>
                <div style={{ height: "3rem", margin: "1rem", display: "flex", justifyContent: "flex-end", gap: "1rem" }}>
                    {["Admin", "Inspector"].includes(activeUser?.role) && (
                        <Button
                            label="Create Report"
                            className="p-button-primary"
                            onClick={() => {
                                setShowAddForm(true);
                                navigate("report", { state: { selectedCarData: selectedCarData } });
                            }}
                        />
                    )}
                </div>
                ...
                <MuiTable
                    tableTitle="Car Inspection Reports"
                    tableData={data?.data}
                    tableColumns={columns}
                    handleDelete={(e, item_id) => handleDelete(e, item_id)}
                    hideRowEdit={(rowData) => {
                        return rowData?.spatie_current_status?.name ? true : false;
                    }}
                    showDelete={["Admin", "Vendor"].includes(loggedInUserData?.role)}
                ...
                <InspectionReportPdfModal selectedItem={selectedItem} loggedInUserData={props?.loggedInUserData} show={showPDF} onHide={() => setShowPDF(false)} onClose={onFormClose} />
