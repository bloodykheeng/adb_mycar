...
import { getAllParkingFees, getParkingFeeById, postParkingFees, updateParkingFees, deleteParkingFeeById } from "../../../services/parking/parking-fee-service.js";
...
function RowForm({ handleSubmit, selectedParentItem, initialData = { name: "", details: "", photoUrl: "" }, ...props }) {
    ...
    const validate = (values) => {
        ...
        if (!values.car_id) errors.car_id = "Car is required";
        if (!values.parking_fee_id) errors.parking_fee_id = "Parking Fee is required";
    ...
    //
    //==================== Vendors ============
    const [selectedVendor, setSelectedVendor] = useState(initialData?.vendors?.vendor ?? selectedParentItem);
    ...
    //================================== Cars (Cars) =============================
    const [selectedCar, setSelectedCar] = useState(initialData?.cars?.Car ?? selectedParentItem);
    const [filteredCar, setFilteredCar] = useState();

    const getListOfCars = useQuery(["cars"], () => getAllCars(), {
        onSuccess: (data) => {},
        onError: (error) => {
    ...
    if (selectedParentItem) {
        initialData = { ...initialData, car_id: selectedParentItem?.id };
    }

    //================================== Parking Fees (ParkingFees) =============================
    const [selectedParkingFee, setSelectedParkingFee] = useState(initialData?.parkingFees?.ParkingFee ?? selectedParentItem);
    const [filteredParkingFee, setFilteredParkingFee] = useState();

    const getListOfParkingFees = useQuery(["parkingFees"], () => getAllParkingFees(), {
        onSuccess: (data) => {},
        onError: (error) => {
            console.log("Error fetching Parking Fees: ", error);
            error?.response?.data?.message ? toast.error(error?.response?.data?.message) : !error?.response ? toast.warning("Check Your Internet Connection Please") : toast.error("An Error Occured Please Contact Admin");
        },
    });

    if (selectedParentItem) {
        initialData = { ...initialData, parking_fee_id: selectedParentItem?.id };
    }
            ...
            <div className="card p-fluid">
                <Form
                    ...
                    render={({ handleSubmit, form, submitting, pristine, values }) => (
                        ...
                        >
                            <Field name="parking_fee_id">
                                {({ input, meta }) => (
                                    <div className="p-field m-4">
                                        <label htmlFor="parking_fee_id">Parking Fee</label>
                                        <AutoComplete
                                            value={selectedParkingFee?.name || ""}
                                            suggestions={filteredParkingFee}
                                            disabled={getListOfParkingFees.isLoading}
                                            completeMethod={(e) => {
                                                const results = getListOfParkingFees.data?.data?.filter((item) => {
                                                    return item.name.toLowerCase().includes(e.query.toLowerCase());
                                                });
                                                setFilteredParkingFee(results);
                                            }}
                                            field="name"
                                            dropdown={true}
                                            onChange={(e) => {
                                                if (typeof e.value === "string") {
                                                    ...
                                                    setSelectedParkingFee({ name: e.value });
                                                    input.onChange("");
                                                } else if (typeof e.value === "object" && e.value !== null) {
                                                    // Update the selected item and set the form state with the selected item's ID
                                                    setSelectedParkingFee(e.value);
                                                    input.onChange(e.value.id);
                                                }
                                            }}
                                            id="parking_fee_id"
                                            selectedItemTemplate={(value) => <div>{value ? value.name : "Select a Parking Fee"}</div>}
                                        />
                                        {meta.touched && meta.error && <small className="p-error">{meta.error}</small>}
                                        {getListOfParkingFees.isLoading && <ProgressSpinner style={{ width: "10px", height: "10px" }} strokeWidth="4" />}
                                    </div>
                                )}
                            </Field>
                                ...
                                {({ input, meta }) => (
                                    <div className="p-field m-4">
                                        ...
                                        <AutoComplete
                                            ...
                                            suggestions={filteredCar}
                                            disabled={getListOfCars.isLoading}
                                            completeMethod={(e) => {
                                                const results = getListOfCars.data?.data?.filter((item) => {
                                                    return item.name.toLowerCase().includes(e.query.toLowerCase());
                                                });
                                                setFilteredCar(results);
                                            }}
                                            field="name"
                                            dropdown={true}
                                            onChange={(e) => {
                                            ...
                                            id="car_id"
                                            selectedItemTemplate={(value) => <div>{value ? value.name : "Select a Car"}</div>}
                                        />
                                        {meta.touched && meta.error && <small className="p-error">{meta.error}</small>}
                                        {getListOfCars.isLoading && <ProgressSpinner style={{ width: "10px", height: "10px" }} strokeWidth="4" />}
                                    </div>
                                )}
                            </Field>
